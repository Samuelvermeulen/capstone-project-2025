...
def merge_strikers_files():
    """
    Fusionne tous les fichiers PL_strickers_*.csv du dossier 'Projet python data'
    Transformation : colonne AC (vide/blanche) - garder uniquement les 2 premières lettres
    """
    ...
    # Trouver les fichiers PL_strickers_*.csv
    strikers_files = glob.glob(os.path.join(project_folder, "PL_strickers_*.csv"))
    strikers_files.sort()  # Trier les fichiers
    
    print(f"🎯 Fichiers PL_strickers trouvés : {[os.path.basename(f) for f in strikers_files]}")
    
    if not strikers_files:
        print("❌ Aucun fichier PL_strickers_*.csv trouvé")
        return None
    ...
    # ÉTAPE 2 : TRANSFORMATION COLONNE AC
    print("\n" + "="*50)
    print("ÉTAPE 2 : TRANSFORMATION COLONNE AC")
    print("="*50)
    
    # Colonne AC = index 28 (A=0, B=1, C=2, ..., AC=28)
    target_col_index = 28
    
    if len(merged_df.columns) > target_col_index:
        target_col_name = merged_df.columns[target_col_index]
        print(f"🔧 Transformation colonne AC (index {target_col_index} - '{target_col_name}') : garder 2 premières lettres")
        ...
        print(f"   📊 Aperçu avant transformation :")
        sample_values = merged_df[target_col_name].head(5).tolist()
        for i, val in enumerate(sample_values):
            if pd.notna(val) and val != '':
                print(f"      '{val}' → '{str(val)[:2]}'")
            else:
                print(f"      {val} → (inchangé)")
        
        merged_df[target_col_name] = merged_df[target_col_name].apply(keep_first_two_chars)
        print(f"   ✅ Colonne AC transformée (2 premières lettres conservées)")
        
        # Aperçu après transformation
        print(f"   📊 Aperçu après transformation :")
        sample_values_after = merged_df[target_col_name].head(5).tolist()
        for val in sample_values_after:
            print(f"      '{val}'")
            
    else:
        print(f"❌ Colonne AC (index {target_col_index}) non trouvée")
        print(f"   Noms de colonnes disponibles : {list(merged_df.columns)}")
    ...
    output_path = os.path.join(desktop_path, "Dataset_creation", "Merged_Dataset", "PL_strickers.csv")
    ...
    print(f"🎯 Fichiers fusionnés : {len(strikers_files)}")
    
    if len(merged_df.columns) > target_col_index:
        target_col_name = merged_df.columns[target_col_index]
        print(f"🔧 Transformation appliquée :")
        print(f"   - Colonne AC ('{target_col_name}') : 2 premières lettres conservées")
...
if __name__ == "__main__":
    print("🚀 DÉBUT DE LA FUSION DES FICHIERS PL_STRICKERS")
    print("=" * 60)
    final_data = merge_strikers_files()
    
    if final_data is not None:
        print(f"\n🎉 OPÉRATION RÉUSSIE !")
        print(f"📁 Le fichier PL_strickers.csv a été créé sur votre Bureau")
