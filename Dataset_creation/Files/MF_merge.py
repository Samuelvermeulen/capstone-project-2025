...
def merge_mid_files():
    """
    Fusionne tous les fichiers PL_MID_*.csv du dossier 'Projet python data'
    Transformation : colonne 42 (Unnamed: 40 ou colonne AO) - garder uniquement les 2 premières lettres
    """
    ...
    # Trouver les fichiers PL_MID_*.csv
    mid_files = glob.glob(os.path.join(project_folder, "PL_MID_*.csv"))
    mid_files.sort()  # Trier les fichiers
    
    print(f"🎯 Fichiers PL_MID trouvés : {[os.path.basename(f) for f in mid_files]}")
    
    if not mid_files:
        print("❌ Aucun fichier PL_MID_*.csv trouvé")
        return None
    ...
    # ÉTAPE 2 : TRANSFORMATION COLONNE 42
    print("\n" + "="*50)
    print("ÉTAPE 2 : TRANSFORMATION COLONNE 42")
    print("="*50)
    
    # Méthode 1 : Chercher par nom "Unnamed: 40"
    target_col_name = None
    
    # Chercher "Unnamed: 40" dans les noms de colonnes
    for col_name in merged_df.columns:
        if "Unnamed: 40" in str(col_name):
            target_col_name = col_name
            break
    
    # Méthode 2 : Si pas trouvé, utiliser l'index 41 (colonne 42)
    if target_col_name is None and len(merged_df.columns) > 41:
        target_col_name = merged_df.columns[41]
    
    # Appliquer la transformation
    if target_col_name is not None:
        print(f"🔧 Transformation colonne '{target_col_name}' : garder 2 premières lettres")
        ...
        print(f"   ✅ Colonne transformée (2 premières lettres conservées)")
        ...
        print(f"❌ Colonne 42 non trouvée")
    ...
    output_path = os.path.join(desktop_path, "Dataset_creation", "Merged_Dataset", "PL_MID.csv")
    ...
    print(f"🎯 Fichiers fusionnés : {len(mid_files)}")
    
    if target_col_name:
        print(f"🔧 Transformation appliquée :")
        print(f"   - Colonne '{target_col_name}' : 2 premières lettres conservées")
...
if __name__ == "__main__":
    print("🚀 DÉBUT DE LA FUSION DES FICHIERS PL_MID")
    print("=" * 60)
    final_data = merge_mid_files()
    
    if final_data is not None:
        print(f"\n🎉 OPÉRATION RÉUSSIE !")
        print(f"📁 Le fichier PL_MID.csv a été créé sur votre Bureau")
